...
class UIXMLTree:
    ...
    def process(self, xml_string, app_info=None, level=1, str_type="json", remove_system_bar=True, use_bounds=False,
        ...
        if "json" == str_type:
            ...
            self.id_mapping = {}
            xml_string = self.dict_to_plain_text(self.xml_dict)
            return xml_string, self.id_mapping
        else:
        ...
        for key, value in xml_dict.items():
            ...
            if not key.startswith("[n_"):
                if isinstance(value, dict):
                    result += self.dict_to_plain_text(value, indent)
                continue
            ...
            if isinstance(value, dict) and "bounds" in value:
                ...
                if m:
                    node_id = m.group(1)
                    coords = [int(x) for x in re.findall(r"\d+", value["bounds"])]
                    self.id_mapping[node_id] = coords

            if isinstance(value, dict):
                result += self.dict_to_plain_text(value, indent + 4)
    ...
    def get_xpath(self, node):
        if node.tag == 'hierarchy':
            ...
            if rid:
                # 如果值里有单引号，就用双引号；否则用单引号
                if "'" in rid and '"' not in rid:
                    my_path = f'//*[@resource-id="{rid}"]'
                elif '"' in rid and "'" not in rid:
                    my_path = f"//*[@resource-id='{rid}']"
                else:
                    # 两种引号都有的话，用 concat() 方式拼接
                    parts = rid.split("'")
                    expr = " , \"'\" , ".join(f"'{p}'" for p in parts)
                    my_path = f"//*[@resource-id=concat({expr})]"
                try:
            ...
            if isinstance(value, dict):
                if "rotation" in value:
                    if not self.app_name:
                        # app_name = f"The screenshot's description is shown"
                        app_name = f"Tree structure description of the current screenshot is shown"
