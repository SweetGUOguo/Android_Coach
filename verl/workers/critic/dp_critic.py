...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # For each sequence, compute the mean of the values and broadcast it back to the sequence length
                new_values_rmpad = torch.empty_like(values_rmpad)
                for i in range(len(cu_seqlens) - 1):
                    start, end = cu_seqlens[i], cu_seqlens[i+1]
                    new_values_rmpad[start:end] = values_rmpad[start:end].mean()
                values_rmpad = new_values_rmpad
    ...
    def update_critic(self, data: DataProto):
        ...
        if self.config.get("use_prm", False):
            select_keys = ["input_ids", "responses", "response_mask", "attention_mask", "position_ids", "values", "step_return_label"]
        else:
            select_keys = ["input_ids", "responses", "response_mask", "attention_mask", "position_ids", "values", "step_return_label"]
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # change return keys
                    # Ensure there is a unified 'returns' key. Map from returns_include_prm or returns_wo_prm if present.
                    if "returns" not in model_inputs:
                        if "step_return_label" in model_inputs:
                            model_inputs["returns"] = model_inputs["step_return_label"]
                        else:
                            model_inputs["returns"] = model_inputs["step_return_label"]
