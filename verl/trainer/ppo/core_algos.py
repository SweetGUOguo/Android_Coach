...
@register_adv_est(AdvantageEstimator.ACLOO)  # or simply: @register_adv_est("rloo")
def compute_acloo_outcome_advantage(
    ...
    **kwargs,
    ...
    """
    Compute advantage for ACLOO based on MY PAPER
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    # Compute masked mean of token-level values as Q(s,a) estimate
    mask_sum = response_mask.sum(dim=-1).clamp(min=1)
    scores = (token_level_values * response_mask).sum(dim=-1) / mask_sum
