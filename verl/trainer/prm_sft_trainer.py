...
from transformers import AutoConfig, AutoModelForVision2Seq, PreTrainedModel

import verl.utils.hdfs_io as hdfs_io
from verl.utils.checkpoint.checkpoint_manager import find_latest_ckpt_path, get_checkpoint_tracker_filename
from verl.utils.checkpoint.fsdp_checkpoint_manager import FSDPCheckpointManager
from verl.utils.dataset import SFTDataset
from verl.utils.dataset.multiturn_sft_dataset import PRMwoHistoryWThoughtSFTDataset
...
class FSDPSFTTrainer:
    def __init__(
        ...
        total_params = sum(p.numel() for p in self.fsdp_model.parameters())
        trainable_params_count = sum(p.numel() for p in self.fsdp_model.parameters() if p.requires_grad)

        print(f"Total model parameters: {total_params / 1_000_000:.2f} M")
        print(f"Trainable parameters: {trainable_params_count / 1_000_000:.2f} M")
    ...
    def _build_model_optimizer(self):
        ...
        optim_strategy = self.config.optim.strategy
        if optim_strategy == 'adamw':
        ...
        elif optim_strategy == "adamw_bf16":
...
def create_sft_dataset(data_paths, data_config, tokenizer):
    """Create a dataset."""
    dataset_cls = PRMwoHistoryWThoughtSFTDataset

    # Create datasets based on the selected class
    dataset = dataset_cls(csv_file=data_paths, config=data_config)
    return dataset
